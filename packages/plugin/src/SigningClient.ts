...
export class SigningClient {
  ...
  async waitForWallet(
    ...
    throw new Error(
      `Timed out waiting for wallet connection after ${timeout / 1000}s. ` +
  ...
  sendSigningRequest(
    ...
    this._httpPost("/api/sign", request, 300_000)
      .then((response) => callback(response as SigningResponse))
      .catch((err) =>
        callback({
          id: request.id,
          success: false,
  ...
  private _httpGet(path: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      http
        ...
        .on("error", (err) =>
          reject(new Error(`Cannot reach AirSign server: ${err.message}`))
        );
    });
  }
        ...
        (res) => {
          ...
          res.on("end", () => {
            try {
              ...
              reject(
                new Error(`Invalid JSON response from server: ${resData}`)
              );
            }
          });
        }
